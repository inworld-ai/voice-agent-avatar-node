  ...
  private async processAudioChunk(message: any, sessionId: string) {
    try {
      ...
      if (!connection.audioStreamManager) {
        ...
        // Graph will be created lazily if it doesn't exist yet with session-specific voiceId
        const graphWrapper = await this.inworldApp.getGraphForSTTService(
          connection.sttService,
          sessionId, // Pass sessionId to get session-specific voiceId
        );
    ...
    try {
      ...
      for await (const result of outputStream) {
        ...
        if (result && result.isGraphError && result.isGraphError()) {
          ...
          if (isTimeout) {
            console.error(
              `[Session ${sessionId}] TIMEOUT DETECTED - Closing audio session`,
            );
    ...
    try {
      await result.processResponse({
        ...
        error: async (error: GraphTypes.GraphError) => {
          ...
          if (isTimeout) {
            console.error(
              `[Session ${sessionId}] NODE TIMEOUT DETECTED - Closing audio session`,
              "\n  Possible causes:",
