...
export const History = (props: HistoryProps) => {
  ...
  useEffect(() => {
    if (ref.current && combinedChatHistory.length > 0) {
      const isNewMessage =
        combinedChatHistory.length > prevHistoryLengthRef.current;
  ...
  useEffect(() => {
      ...
      switch (item.type) {
        case CHAT_HISTORY_TYPE.ACTOR:
          ...
          if (currentRecord) {
            ...
            if (
              currentRecord.messages.length === 1 &&
              currentRecord.source.name === "Assistant" &&
              item.source.name !== "Assistant"
            ) {
    ...
    const userMessages = history.filter(
      (item) =>
        item.type === CHAT_HISTORY_TYPE.ACTOR && item.source?.isUser === true,
    );
    ...
    for (let i = userMessages.length - 1; i >= 0; i--) {
      ...
      const hasAgentResponse = mergedRecords.some(
        (item) =>
          ...
          item.source.isAgent &&
          item.messages.some(
            (m) => !m.isRecognizing && m.text && m.text.trim().length > 0,
          ),
      );
    ...
    if (pendingUserInteractionId) {
      ...
      const placeholderExists = mergedRecords.some(
        (item) =>
          item.interactionId === pendingUserInteractionId &&
          item.source.isAgent &&
          item.messages.length === 0,
      );

      if (!placeholderExists) {
        // Find the last agent message to reuse its name
        const lastAgentRecord = mergedRecords
          .slice()
          .reverse()
          .find((record) => record.source?.isAgent);

        const placeholderItem: CombinedHistoryItem = {
          interactionId: pendingUserInteractionId,
          messages: [],
          source: {
            name: lastAgentRecord?.source?.name || "Assistant",
            isAgent: true,
            isUser: false,
          } as Actor,
          type: CHAT_HISTORY_TYPE.ACTOR,
